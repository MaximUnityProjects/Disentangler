    ...
    public class DynamicDecals : MonoBehaviour
    {
        ...
        private void PreShaderReplacement(Camera Camera, CameraData Data)
        {
            if (Projections.Count > 0 && shaderReplacement)
            {
                ...
                if (UnityEngine.XR.XRSettings.enabled && Settings.SinglePassVR && Camera.stereoEnabled)
                {
                    //Left eye
        ...
        {
            int width = (UnityEngine.XR.XRSettings.enabled && OriginalCamera.stereoEnabled && Settings.SinglePassVR) ? UnityEngine.XR.XRSettings.eyeTextureWidth * 2: CustomCamera.pixelWidth;
