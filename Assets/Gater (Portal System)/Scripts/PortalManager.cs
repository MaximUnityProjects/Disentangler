...
public class PortalManager : MonoBehaviour {
	...
	[Serializable] public class PortalSettingsClass {
		...
		public Material CloneObjectClipPlaneMaterial;
	}
	...
	private Shader ClipPlaneShader;
	...
	void OnEnable () {
		...
		if (!ClipPlaneMaterial)
			ClipPlaneShader = Shader.Find ("Gater/ClipPlane");

		if (!ClipPlaneMaterial)
			ClipPlaneMaterial = new Material (ClipPlaneShader);
		...
		if (PortalFunctionality.ExcludedObjsFromTrigger.Length > 0)
			for (int j = 0; j < PortalFunctionality.ExcludedObjsFromTrigger.Length; j++)
				if (PortalFunctionality.ExcludedObjsFromTrigger [j])
					if (!PortalFunctionality.ExcludedObjsFromTrigger [j].GetComponent<Collider> ())
		...
		if (!PairedPortal || PairedPortal.GetComponent<PortalManager> ().PairedPortal != gameObject) {
			...
			if (!InGameCamera) {
					...
					if (j < ViewSettings.Recursion.Steps + 1) {
						if (!PortalCamObjs [j]) {
							...
							if (InGameCamera) {
								...
								PortalCamObjs [j].GetComponent<Camera> ().allowMSAA = InGameCamera.allowMSAA;
							}
						}

						if (PairedPortal.GetComponent<PortalManager> ().PortalCamObjs [j])
							PairedPortal.GetComponent<PortalManager> ().PortalCamObjs [j].GetComponent<Skybox> ().material = ViewSettings.Recursion.CustomFinalStep && (j > 0 && j == ViewSettings.Recursion.Steps) ? ViewSettings.Recursion.CustomFinalStep : (!PairedPortalSkybox && (j > 0 && j == ViewSettings.Recursion.Steps) ? null : PairedPortalSkybox);
					} else {
						#if UNITY_EDITOR
							if (!EditorApplication.isPlaying)
				...
				if (PairedPortal) {
                    ...
                    //if (PairedPortal.GetComponent<PortalManager>().PortalSettings.EnableRender && GetComponent<Renderer>().isVisible) {
	...
	void OnTriggerEnter (Collider collision) {
		...
		if (CheckExcludedList (collision.gameObject, false))
			return;

		//Increment and partially fill the arrays elements of required object for teleport
		if (collision.gameObject != gameObject && !collision.GetComponent<PortalManager> () && !collision.name.Contains (collision.gameObject.GetHashCode ().ToString ()) && !collision.name.Contains ("Clone")) {
						...
						if (CollidedObjsAlwaysTeleport [i]) {
							if (!CloneCollidedObjs [i]) {
								...
								if (CloneCollidedObjs [i].GetComponent<MeshRenderer> ()) {
									if (PortalSettings.EnableClipPlane) {
										if (PortalSettings.CloneObjectClipPlaneMaterial) {
											if (PortalSettings.CloneObjectClipPlaneMaterial.shader == ClipPlaneShader)
												CloneCollidedObjs [i].GetComponent<MeshRenderer> ().sharedMaterial = PortalSettings.CloneObjectClipPlaneMaterial;
											if (PortalSettings.CloneObjectClipPlaneMaterial.shader != ClipPlaneShader)
												Debug.LogError ("CloneObjectClipPlaneMaterial contains a wrong shader");
										}
										if (!PortalSettings.CloneObjectClipPlaneMaterial)
											Debug.LogError ("CloneObjectClipPlaneMaterial it's null");
									}
									if (!PortalSettings.EnableClipPlane) {
							...
							if (CloneCollidedObjs [i]) {
								...
								if (CollidedObjs [i].GetComponent<MeshRenderer> () && CollidedObjs [i].GetComponent<MeshRenderer> ().sharedMaterial == ClipPlaneMaterial) {
									...
									CollidedObjs [i].GetComponent<MeshRenderer> ().sharedMaterial.SetVector ("_PlanePoint", ClipPlanePosObj.transform.position);
									CollidedObjs [i].GetComponent<MeshRenderer> ().sharedMaterial.SetVector ("_PlaneNormal", Quaternion.Euler (transform.eulerAngles) * -DirectionVector);

									CloneCollidedObjs [i].GetComponent<MeshRenderer> ().sharedMaterial.SetVector ("_PlanePoint", PairedPortal.GetComponent<PortalManager> ().ClipPlanePosObj.transform.position);
									CloneCollidedObjs [i].GetComponent<MeshRenderer> ().sharedMaterial.SetVector ("_PlaneNormal", Quaternion.Euler (PairedPortal.transform.eulerAngles) * -DirectionVector);
								}
							}
						}
					}
		...
		for (int i = 0; i < CloneCollidedObjs.Length; i++) {
			if (ContinueTriggerEvents [i] && CollidedObjs [i] && CollidedObjs [i].GetHashCode ().ToString () == collision.gameObject.GetHashCode ().ToString () && CloneCollidedObjs [i]) {
				...
				if (CollObjProxDetDistExit > 0) {
					...
					bool CollidedObjFPS = CollidedObjs [i].GetComponent<UnityStandardAssets.Characters.FirstPerson.FirstPersonController> ();
				...
				if (CollidedObjs [i].GetComponent<MeshRenderer> () && PortalSettings.EnableClipPlane) {
					CollidedObjs [i].GetComponent<MeshRenderer> ().sharedMaterials = CollidedObjsInitMaterials [i].Materials;
	...
	bool CheckExcludedList (GameObject collision, bool Excluded) {
		if (PortalFunctionality.ExcludedObjsFromTrigger.Length > 0)
			for (int i = 0; i < PortalFunctionality.ExcludedObjsFromTrigger.Length; i++)
				if (PortalFunctionality.ExcludedObjsFromTrigger [i] && PortalFunctionality.ExcludedObjsFromTrigger [i] == collision)
					return true;
